        ...
        if (!$is_admin && $user_id) {
            ...
            if ($team_ids) {
                ...
                foreach ($teams_array as $team_id) {
                    $team_search_sql .= " OR (FIND_IN_SET('team:$team_id', $google_docs_table.share_with_team_members)) ";
                }
            }
            ...
            if ($is_client) {
                ...
                $where .= " AND (FIND_IN_SET('all', $google_docs_table.share_with_client_contacts) OR FIND_IN_SET('contact:$user_id', $google_docs_table.share_with_client_contacts))";
            } else {
                //searh for user and teams
                $where .= " AND ($google_docs_table.created_by=$user_id 
                OR $google_docs_table.share_with_team_members='all'
                    OR (FIND_IN_SET('member:$user_id', $google_docs_table.share_with_team_members))
                        $team_search_sql
                        )";
            }
