...
public function index() {
    ...
    // Projects dropdown
    $projects = $this->projectsModel->get_all_where(["deleted" => 0])->getResult();
    $projects_dropdown = ["" => "- " . app_lang('project') . " -"];
    ...
    // Members dropdown
    $members = $this->Users_model->get_all_where(["user_type" => "staff", "deleted" => 0, "status" => "active"])->getResult();
    $members_dropdown = ["" => "- " . app_lang('member') . " -"];
    ...
    // Tasks dropdown
    $tasks_where = ["deleted" => 0];
    if ($project_id) $tasks_where["project_id"] = $project_id;
    $tasks = $this->Tasks_model->get_all_where($tasks_where)->getResult();
    $tasks_dropdown = ["" => "- " . app_lang('task') . " -"];
    ...
    $project_report_where = $project_id ? " AND $projects_table.id = $project_id" : "";
    $tasks_join_where = "";
    if ($member_id) $tasks_join_where .= " AND $tasks_table.assigned_to = $member_id";
    if ($task_id) $tasks_join_where .= " AND $tasks_table.id = $task_id";
    ...
    // Time tracking report (fixed Spent Hr)
    $users_table = $this->db->prefixTable('users');
    $project_members_table = $this->db->prefixTable('project_members');
    $project_time_table = $this->db->prefixTable('project_time');

    $time_tracking_where = "";
    if ($project_id) $time_tracking_where .= " AND p.id = $project_id";
    if ($member_id) $time_tracking_where .= " AND u.id = $member_id";

    $est_where = "WHERE deleted = 0";
    if ($task_id) $est_where .= " AND id = $task_id";

    $spt_where = "WHERE deleted = 0 AND status = 'logged'";
    if ($task_id) $spt_where .= " AND task_id = $task_id";
    ...
    $sql_time = "
        SELECT
            ...
            COALESCE(est.total_estimated_hr, 0) AS total_estimated_hr,
            COALESCE(spt.total_spent_seconds, 0) AS total_spent_seconds
        FROM
            $users_table u
        JOIN
        ...
        LEFT JOIN
            (SELECT assigned_to, project_id, SUM(estimated_time) AS total_estimated_hr 
             FROM $tasks_table $est_where 
             GROUP BY assigned_to, project_id) AS est
             ON est.assigned_to = u.id AND est.project_id = p.id
        LEFT JOIN
            (SELECT user_id, project_id, SUM(
                COALESCE(TIME_TO_SEC(TIMEDIFF(end_time, start_time)), 0) + COALESCE(hours * 3600, 0)
            ) AS total_spent_seconds
             FROM $project_time_table $spt_where 
             GROUP BY user_id, project_id) AS spt
    ...
    foreach ($view_data['time_tracking_report_data'] as $item) {
        $total_time_logged_seconds += (float) $item->total_spent_seconds; // cast to float to avoid concatenation
    }
    $view_data['total_time_logged'] = $this->format_to_time($total_time_logged_seconds);

    // User time log report
    $user_time_log_where = "";
    if ($project_id) $user_time_log_where .= " AND $project_time_table.project_id = $project_id";
    if ($member_id) $user_time_log_where .= " AND $project_time_table.user_id = $member_id";
    if ($task_id) $user_time_log_where .= " AND $project_time_table.task_id = $task_id";
    $project_time_table = $this->db->prefixTable('project_time');
    $sql_user_time_log = "
        SELECT
            ...
            COALESCE(TIME_TO_SEC(TIMEDIFF($project_time_table.end_time, $project_time_table.start_time)), 0) 
            + COALESCE($project_time_table.hours * 3600, 0) AS spent_seconds
        FROM
            $project_time_table
        JOIN
...
// Helper function for formatting seconds to HH:MM
private function format_to_time($seconds) {
